...
def extract_n_images_from_labels(annotations_file, n, labels, target_file, single_only: bool):
    result = pd.DataFrame()
    annotations = pd.read_csv(annotations_file)
    if single_only:
        annotations = annotations[annotations["Finding Labels"].str.contains("\|") == False]
    for label in labels:
        result = pd.concat([result, annotations[annotations["Finding Labels"].str.contains(label) == True].tail(n)])
